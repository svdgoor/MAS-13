...
globals [
  ...
  quadrant-1-modifier  ;; Modifier for the top-left quadrant ADDED EXTRA
  quadrant-2-modifier  ;; Modifier for the top-right quadrant
  quadrant-3-modifier  ;; Modifier for the bottom-left quadrant
  quadrant-4-modifier  ;; Modifier for the bottom-right quadrant UNTIL HERE
]
...
to-report get-quadrant
  ;; Divide the world into 4 quadrants based on patch coordinates
  let mid-x (max-pxcor + min-pxcor) / 2
  let mid-y (max-pycor + min-pycor) / 2

  if xcor <= mid-x and ycor > mid-y [report 1] ;; Top-left
  if xcor > mid-x and ycor > mid-y [report 2] ;; Top-right
  if xcor <= mid-x and ycor <= mid-y [report 3] ;; Bottom-left
  if xcor > mid-x and ycor <= mid-y [report 4] ;; Bottom-right
end
...
to reproduce  ;; turtle procedure THIS IS ADDED FOR GABRIELS PART
  let quadrant get-quadrant
  let modifier 1

  ;; Assign the modifier based on the turtle's quadrant
  if quadrant = 1 [ set modifier quadrant-1-modifier ]
  if quadrant = 2 [ set modifier quadrant-2-modifier ]
  if quadrant = 3 [ set modifier quadrant-3-modifier ]
  if quadrant = 4 [ set modifier quadrant-4-modifier ]

  ;; Apply the modifier to the reproduction probability
  if random-float 1.0 < (ptr * modifier) [
